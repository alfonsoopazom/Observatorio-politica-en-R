...
carpeta<-"C:/Users/interbarometro/Desktop/ProgramasR/Limpiador Cipolapp"
...
for(i in 1:length(nombres[,1])){
  ...
  #manzanapp no trae la columna sexo
  
  respuesta <- sqldf('SELECT DISTINCT(user_id), status_id, created_at, screen_name,text,source,display_text_width,
                      reply_to_status_id,reply_to_user_id,reply_to_screen_name,is_quote,is_retweet,favorite_count,
                      retweet_count,quote_count,reply_count,hashtags,symbols,urls_url,"urls_t.co" "urls_t.co",urls_expanded_url,
